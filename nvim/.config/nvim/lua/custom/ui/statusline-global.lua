...
Statusline.build = function()
  ...
  local cwd = string.format('%s %s ', (vim.g.have_nerd_font and ' ' or 'CWD: '), vim.fn.fnamemodify(vim.fn.getcwd(), ':t'))
  ...
  local diagnostics = {}
  if errors > 0 then
    table.insert(diagnostics, '%#DiagnosticError#E:' .. errors)
  end
  if warnings > 0 then
    table.insert(diagnostics, '%#DiagnosticWarn#W:' .. warnings)
  end
  if info > 0 then
    table.insert(diagnostics, '%#DiagnosticInfo#I:' .. info)
  end
  if hints > 0 then
    table.insert(diagnostics, '%#DiagnosticHint#H:' .. hints)
  end

  local diagnostics_str = ''
  if #diagnostics > 0 then
    diagnostics_str = ' ' .. table.concat(diagnostics, ' ') .. ' '
  end
