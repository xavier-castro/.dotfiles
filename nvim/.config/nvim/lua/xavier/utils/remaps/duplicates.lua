local functions = require("xavier.utils.functions")
    ...
    if check then
        ...
        if found ~= nil then
            if found ~= description then
                print(input .. " already mapped (" .. found ..
                    " so we cannot map (" .. description .. ")")
            end
        end
...
M.check_duplicates = function(type, input, description)
    ...
    if functions.is_table(type) then
        if type["n"] then check_n = true end
        if type["v"] then check_v = true end
        if type["i"] then check_i = true end
        if type["s"] then check_s = true end
        if type["x"] then check_x = true end
    else
        if type == "n" then check_n = true end
        if type == "v" then check_v = true end
        if type == "i" then check_i = true end
