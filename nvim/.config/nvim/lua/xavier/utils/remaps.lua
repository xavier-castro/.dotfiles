...
local check_duplicates =
    require("xavier.utils.remaps.duplicates").check_duplicates

local M = {}

local which_key_lazy_registers = nil
local function lazy_register_which_key(input, description)
    if which_key_lazy_registers == nil then which_key_lazy_registers = {} end
...
local function try_add_to_which_key_by_input(input, description)
    ...
    if has_leader then
        if present_which_key then
            ...
            which_key.register({ [input] = description })
        else
