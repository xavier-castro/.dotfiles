...
function M.is_table(to_check) return type(to_check) == "table" end

function M.has_key(t, key)
    for t_key, _ in pairs(t) do if t_key == key then return true end end

    return false
end

function M.has_value(t, val)
    for _, value in ipairs(t) do if value == val then return true end end
...
function M.tprint_keys(table) for k in pairs(table) do print(k) end end
...
function M.is_macunix() return vim.fn.has("macunix") end
...
M.highlight = function(group, opts) vim.api.nvim_set_hl(0, group, opts) end

M.highlight_bg =
    function(group, col) vim.api.nvim_set_hl(0, group, { bg = col }) end

-- Define fg color
-- @param group Group
-- @param color Color
M.highlight_fg =
    function(group, col) vim.api.nvim_set_hl(0, group, { fg = col }) end
