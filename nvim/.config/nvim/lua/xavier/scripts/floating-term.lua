  ...
  if line_count <= 1 and vim.api.nvim_buf_get_lines(float_term_buf, 0, -1, false)[1] == "" then
    ...
    local job_id = vim.fn.termopen(shell, {
      ...
      on_exit = function(_, exit_code)
        ...
        if exit_code == 0 then float_term_buf = nil end
      end,
    })
...
vim.keymap.set("t", "<C-x>", function()
  if float_term_win and vim.api.nvim_win_is_valid(float_term_win) then close_floating_terminal() end
end, {
...
vim.api.nvim_create_autocmd("VimResized", {
  ...
  callback = function()
    if float_term_win and vim.api.nvim_win_is_valid(float_term_win) then
      ...
      if not ok then vim.notify("Failed to resize floating terminal: " .. err, vim.log.levels.WARN) end
    end
  end,
})
