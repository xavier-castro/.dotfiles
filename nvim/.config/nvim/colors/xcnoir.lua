...
-- Forward declare utils table
local utils

-- Define to_rgb separately first
local function to_rgb(color)
	return tonumber(color:sub(2, 3), 16), tonumber(color:sub(4, 5), 16), tonumber(color:sub(6), 16)
end

-- Then create the utils table
utils = {
	to_rgb = to_rgb,

	shade_color = function(color, percent)
		local r, g, b = to_rgb(color) -- Use local to_rgb instead of utils.to_rgb
...
-- Modify your existing cursor highlight to be bright red
hl["Cursor"] = { bg = red2 }

-- Add NormalNC (Normal Non-Current) highlight group for non-focused windows
-- This will make non-focused windows slightly dimmer
hl["NormalNC"] = { fg = gray5, bg = utils.shade_color(gray0, -15) }

-- Make sure Normal is defined for the active window
hl["Normal"] = { fg = gray5, bg = gray0 }
